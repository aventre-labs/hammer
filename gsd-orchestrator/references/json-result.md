...
> Hammer/IAM awareness: JSON result fields are diagnostics for provenance, blockers, and no-degradation remediation.

When using `--output-format json`, Hammer collects events silently and emits a single `HeadlessJsonResult` JSON object to stdout at process exit. This is the structured result for orchestrator decision-making.
...
| `phase` | `string \| undefined` | Current Hammer phase at session end (e.g. `"executing"`, `"blocked"`, `"complete"`). |
