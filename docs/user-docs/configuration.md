...
Hammer preferences live in `~/.gsd/PREFERENCES.md` (global) or `.gsd/PREFERENCES.md` (project-local). Manage interactively with `/hammer prefs`.

> **Internal-implementation paths preserved.** The preference file paths (`~/.gsd/`, `.gsd/PREFERENCES.md`), the auth file (`~/.gsd/agent/auth.json`), and all `GSD_*` environment variables are filesystem identifiers and remain on the GSD identifier verbatim per the rebrand-window scoping rule. Only user-facing prose, slash commands (`/hammer …`), and the chat handle (`@hammer`) are rebranded.

## `/hammer prefs` Commands

| Command | Description |
|---------|-------------|
| `/hammer prefs` | Open the global preferences wizard (default) |
| `/hammer prefs global` | Interactive wizard for global preferences (`~/.gsd/PREFERENCES.md`) |
| `/hammer prefs project` | Interactive wizard for project preferences (`.gsd/PREFERENCES.md`) |
| `/hammer prefs status` | Show current preference files, merged values, and skill resolution status |
| `/hammer prefs wizard` | Alias for `/hammer prefs global` |
| `/hammer prefs setup` | Alias for `/hammer prefs wizard` — creates preferences file if missing |
| `/hammer prefs import-claude` | Import Claude marketplace plugins and skills as namespaced Hammer components |
| `/hammer prefs import-claude global` | Import to global scope |
| `/hammer prefs import-claude project` | Import to project scope |
...
## Global API Keys (`/hammer config`)

Tool API keys are stored globally in `~/.gsd/agent/auth.json` and apply to all projects automatically. Set them once with `/hammer config` — no need to configure per-project `.env` files.
...
1. `/hammer config` saves keys to `~/.gsd/agent/auth.json`
...
Hammer can connect to external MCP servers configured in project files. This is useful for local tools, internal APIs, self-hosted services, or integrations that aren't built in as native Hammer extensions.

### Config file locations

Hammer reads MCP client configuration from these project-local paths:
...
After adding config, verify it from a Hammer session:
...
1. `mcp_servers` — confirms Hammer can see the config file and parse the server entry
...
- Hammer and `gsd-mcp-server` both hydrate supported model and tool keys saved in `~/.gsd/agent/auth.json`, so MCP configs can safely reference them through `${ENV_VAR}` placeholders without committing raw credentials.
...
|----------|---------|-------------|
| `GSD_HOME` | `~/.gsd` | Global Hammer state directory (env var name preserved as filesystem identifier). All paths derive from this unless individually overridden. Affects preferences, skills, sessions, and per-project state. (v2.39) |
...
Hammer resolves models.json with fallback logic:
1. `~/.gsd/agent/models.json` — primary (Hammer)
...
When a model fails to switch (provider unavailable, rate limited, credits exhausted), Hammer automatically tries the next model in the `fallbacks` list.

### Community Provider Extensions

For providers not built into Hammer, community extensions can add full provider support with proper model definitions, thinking format configuration, and interactive API key setup.
...
Controls how Hammer finds and applies skills during auto mode.
...
git:
  ...
  manage_gitignore: true      # set false to prevent Hammer from modifying .gitignore
...
| `manage_gitignore` | boolean | `true` | When `false`, Hammer will not modify `.gitignore` at all — no baseline patterns, no self-healing. Use if you manage your own `.gitignore` |
...
The path can be absolute or relative to the project root. The script runs with a 30-second timeout. Failure is non-fatal — Hammer logs a warning and continues.
...
**How it works:**
1. Milestone completes → Hammer squash-merges the worktree to the main branch
...
If `pr_target_branch` is not set, the PR targets the `main_branch` (or auto-detected main branch). PR creation failure is non-fatal — Hammer logs and continues.

### `github` (v2.39)

GitHub sync configuration. When enabled, Hammer auto-syncs milestones, slices, and tasks to GitHub Issues, PRs, and Milestones.
...
Control what notifications Hammer sends during auto mode:
...
**macOS delivery:** Hammer uses [`terminal-notifier`](https://github.com/julienXX/terminal-notifier) when available, falling back to `osascript`. We recommend installing `terminal-notifier` for reliable notification delivery:
...
For project-specific knowledge (patterns, gotchas, lessons learned), use `.gsd/KNOWLEDGE.md` instead — it's injected into every agent prompt automatically. Add entries with `/hammer knowledge rule|pattern|lesson <description>`.
...
OpenAI service tier preference for supported models. Toggle with `/hammer fast`.
...
Opt-in: search existing issues and PRs before filing from `/hammer forensics`. Uses additional AI tokens.
