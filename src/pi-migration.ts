 ...
 * into Hammer's auth storage. Runs when Hammer has no LLM providers configured,
...
/**
 * Migrate provider credentials from Pi's auth.json into Hammer's AuthStorage.
 *
 * Only runs when Hammer has no LLM provider configured and Pi's auth.json exists.
 * Copies any credentials Hammer doesn't already have. Returns true if an LLM
...
export function migratePiCredentials(authStorage: AuthStorage): boolean {
  try {
    ...
    for (const [providerId, credential] of Object.entries(piData)) {
      ...
      process.stderr.write(`[hammer] Migrated ${isLlm ? 'LLM provider' : 'credential'}: ${providerId} (from Pi)\n`)
    }
