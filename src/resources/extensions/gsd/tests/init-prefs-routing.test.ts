...
import { mapInitPrefsToWizardShape, buildInitPreferencesBody } from "../init-wizard.ts";
...
test("writePreferencesFile — generated fallback body is Hammer-first", async () => {
  const tmp = mkdtempSync(join(tmpdir(), "gsd-init-prefs-routing-"));
  const path = join(tmp, "PREFERENCES.md");
...
test("buildInitPreferencesBody — generated init body is Hammer-first", () => {
  const body = buildInitPreferencesBody();
  assert.match(body, /^# Hammer Skill Preferences/m);
  assert.match(body, /Generated by `\/hammer init`/);
  assert.match(body, /`\/hammer prefs project`/);
  assert.match(body, /IAM-aware execution defaults/);
  assert.doesNotMatch(body, /^# GSD Skill Preferences/m);
  assert.doesNotMatch(body, /~\/\.gsd\/agent\/extensions\/gsd\/docs\/preferences-reference\.md/);
});
