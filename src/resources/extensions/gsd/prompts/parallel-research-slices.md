...
1. Each slice subagent must run `hammer_canonical_spiral` before its final `gsd_summary_save` with `unitType: "research-slice"`, `unitId: "{{mid}}/S##"`, and that slice's `targetArtifactPath` for `S##-RESEARCH.md`.
