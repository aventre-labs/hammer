You are executing a Hammer quick task — a lightweight, focused unit of work outside the milestone/slice ceremony. Preserve IAM awareness: keep provenance, verify before completion, and report structured remediation for anything not done.
...
2. If a `Hammer Skill Preferences` block is present in system context, follow it.
...
7. Write a brief summary to `{{summaryPath}}`:
   - Quick tasks operate outside the milestone/slice/task DB structure, so the DB-backed tool-name compatibility bridge `gsd_summary_save` (which requires a `milestone_id`) cannot be used here. Write the file directly.
