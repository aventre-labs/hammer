# Headless Hammer Milestone Creation

You are creating a Hammer milestone from a provided specification document. This is a **headless** (non-interactive) flow — do NOT ask the user any questions. Wherever the interactive flow would ask the user, make your best-judgment call and document assumptions. Maintain IAM awareness and no-degradation semantics: preserve provenance, requirement coverage, and structured remediation when evidence is missing.
...
Before writing a roadmap, produce `.gsd/REQUIREMENTS.md` as a legacy state bridge projection.
...
6. For each architectural or pattern decision, call the DB-backed tool-name compatibility bridge `gsd_decision_save` — the tool auto-assigns IDs and regenerates `.gsd/DECISIONS.md` automatically.
7. {{commitInstruction}}
...
1. For each milestone, call the DB-backed tool-name compatibility bridge `gsd_milestone_generate_id` to get its ID — never invent milestone IDs manually. Then `mkdir -p .gsd/milestones/<ID>/slices` (legacy state bridge path) for each.
...
4. For any architectural or pattern decisions, call the DB-backed tool-name compatibility bridge `gsd_decision_save` — the tool auto-assigns IDs and regenerates `.gsd/DECISIONS.md` automatically.
...
After deciding each milestone's readiness, immediately write or update `.gsd/DISCUSSION-MANIFEST.json` as a legacy state bridge manifest:
...
- [ ] Written `.gsd/PROJECT.md` legacy state bridge projection
- [ ] Written `.gsd/REQUIREMENTS.md` legacy state bridge projection
- [ ] Written the primary milestone `CONTEXT.md`
- [ ] Called the DB-backed tool-name compatibility bridge `gsd_plan_milestone` for the primary milestone
- [ ] Written `.gsd/DISCUSSION-MANIFEST.json` legacy state bridge manifest with `gates_completed === total`
...
- **Use proper tools** — the DB-backed tool-name compatibility bridge `gsd_plan_milestone` for roadmaps, the DB-backed tool-name compatibility bridge `gsd_decision_save` for decisions, the DB-backed tool-name compatibility bridge `gsd_milestone_generate_id` for IDs
...
- **Naming convention** — always use the DB-backed tool-name compatibility bridge `gsd_milestone_generate_id` for IDs. Directories use bare IDs, files use ID-SUFFIX format.
