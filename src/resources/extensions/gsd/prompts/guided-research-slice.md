...
4. If `hammer_canonical_spiral` returns an IAM error, times out, or cannot provide complete artifacts, stop and report the IAM error/remediation. Do not call `gsd_summary_save` as if slice research completed successfully.
5. Do not replace the native Omega run with prose-only or deferred Omega guidance.

Call `gsd_summary_save` with `milestone_id: {{milestoneId}}`, `slice_id: {{sliceId}}`, `artifact_type: "RESEARCH"`, and the research content — the tool writes the file to disk and persists to DB.
