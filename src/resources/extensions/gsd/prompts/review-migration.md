## Review Migrated .gsd Directory (legacy state bridge)
...
A `/hammer migrate` command for legacy GSD migration just wrote a `.gsd/` directory from an old `.planning` source. Your job is to audit the output and verify it meets Hammer migration standards before the user starts working with it.

### Source
- Old `.planning` directory: `{{sourcePath}}`
- Written `.gsd` legacy state bridge directory: `{{gsdPath}}`
...
- Run `deriveState()` on the `.gsd` legacy state bridge directory (import from `state.ts`, pass the **project root** as basePath)
...
If the overall result is FAIL, explain what needs manual attention. If PASS WITH NOTES, explain what's imperfect but acceptable. If PASS, confirm the `.gsd` legacy state bridge directory is ready for Hammer auto-mode.
