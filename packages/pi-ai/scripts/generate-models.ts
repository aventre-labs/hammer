...
async function generateModels() {
	...
	const toConstName = (providerId: string): string => {
		const normalized = providerId
			.replace(/[^a-zA-Z0-9]+/g, "_")
			.replace(/_+/g, "_")
			.replace(/^_|_$/g, "")
			.toUpperCase();
		const safePrefix = /^[0-9]/.test(normalized) ? `_${normalized}` : normalized;
		return `${safePrefix}_MODELS`;
	};

	const renderModelEntries = (models: Record<string, Model<any>>): string => {
		let output = "{\n";
	...
	const generatedDir = join(packageRoot, "src/models/generated");
	rmSync(generatedDir, { recursive: true, force: true });
	mkdirSync(generatedDir, { recursive: true });

	for (const providerId of sortedProviderIds) {
		const providerModels = providers[providerId];
		const constName = toConstName(providerId);
		const providerOutput = `// This file is auto-generated by scripts/generate-models.ts
// Do not edit manually - run 'npm run generate-models' to update

import type { Model } from "../../types.js";

export const ${constName} = ${renderModelEntries(providerModels)} as const satisfies Record<string, Model<any>>;
`;
		writeFileSync(join(generatedDir, `${providerId}.ts`), providerOutput);
		console.log(`Generated src/models/generated/${providerId}.ts`);
	}

	let indexOutput = `// This file is auto-generated by scripts/generate-models.ts
// Do not edit manually - run 'npm run generate-models' to update
...
`;
	...
	for (const providerId of sortedProviderIds) {
		const constName = toConstName(providerId);
		indexOutput += `import { ${constName} } from "./${providerId}.js";\n`;
	}

	indexOutput += `\nexport const MODELS = {\n`;
	for (const providerId of sortedProviderIds) {
		const constName = toConstName(providerId);
		indexOutput += `\t${JSON.stringify(providerId)}: ${constName},\n`;
	}
	indexOutput += `} as const;\n`;

	writeFileSync(join(generatedDir, "index.ts"), indexOutput);
	console.log("Generated src/models/generated/index.ts");

	const legacyGeneratedPath = join(packageRoot, "src/models.generated.ts");
	if (existsSync(legacyGeneratedPath)) {
		rmSync(legacyGeneratedPath);
		console.log("Removed legacy src/models.generated.ts");
	}
